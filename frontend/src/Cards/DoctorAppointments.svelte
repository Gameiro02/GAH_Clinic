  ...
  async function markAsCompleted() {
    try {
      ...
      if (tokenString) {
        ...
        const response = await fetch(`${DOMAIN}/appointments/finish/`, {
          method: "POST",
          headers: {
  ...
  <div class="card-body items-center text-center p-1">
    ...
    <div class="appointments flex flex-col h-full w-full">
      ...
      {:else if data.upcomingAppointments.length === 0}
        <p class="info text-accent text-center font-bold text-xl mt-16">Não tens nenhuma consulta agendada.</p>
      {:else}
    ...
    <div class="modal-box flex flex-col">
      ...
      <div class="text text-base-content flex-grow">
        ...
        <p class="pt-2 text-center text-gray text-s pb-5">
          ...
          às
          <span class="text-secondary font-bold">{selectedAppointment.time}</span> horas
        </p>
      </div>
