...
<div class="card w-96 bg-base-200 shadow-xl flex-1">
  <div class="card-body items-center text-center p-1">
    <h2 class="card-title text-base-content text-4xl mb-2 mt-4">Próximas Consultas</h2>
            ...
            <tbody>
              {#each data.upcomingAppointments as appointment, index}
                <tr class="hover:bg-primary">
                  <td class="text-primary-content">{appointment.date}</td>
                  <td class="text-primary-content">{appointment.time}</td>
                  <td class="text-primary-content">{appointment.doctorName}</td>
                </tr>
              {/each}
            </tbody>
          </table>
